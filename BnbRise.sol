/**               
Telegram : https://t.me/BnbRiseTokenTG
Website : TBA
...
contract BnbRise is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BnbRiseDividendTracker public dividendTracker;
    ...
    constructor() ERC20("BNB RISE", "BRISE") {
        marketingWallet = payable(0xA1515ecb6e9BF7ccBA46A0cAaBB44FD204824fff);
        devWallet = payable(0x729C6A9064ED40B682658d27265cC45F2875Ca34);
        ...
        dividendTracker = new BnbRiseDividendTracker(
            payable(this),
            router,
...
contract BnbRiseDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    BnbRise public BnbRiseContract;
    ...
    constructor(
        ...
        defaultToken = token;
        BnbRiseContract = BnbRise(mainContract);
    ...
    function _transfer(
        ...
        require(false, "BnbRise_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "BnbRise_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BnbRise contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BnbRiseContract);

            uint256 prevBalance = BnbRiseContract.balanceOf(address(this));
            ...
            {
                uint256 received = BnbRiseContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BnbRiseContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
